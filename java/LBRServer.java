...
public class LBRServer extends RoboticsAPIApplication
{
    ...
    public void initialize()
    {
        ...
        // Sampling frequency
        usrin = appUi.displayModalDialog(
        ...
        getLogger().info("    Send period millisec: " + String.valueOf(friConfig.getSendPeriodMilliSec()));
...
//            if (ansRate > 5) {
//
//              getLogger().warn("Answer rate is " + String.valueOf(ansRate)+ "ms, WRENCH command mode requires <= 5ms. Choose a different sampling frequency.");
//              return;
//            }
//            break;
          case 2:
            ...
            if (ansRate > 5) {
              initSuccess = false;
              getLogger().warn("Answer rate is " + String.valueOf(ansRate)+ "ms, TORQUE command mode requires <= 5ms. Choose a different sampling frequency.");
              return;
            }
            break;
          default:
        ...
        usrin = appUi.displayModalDialog(
            ApplicationDialogType.QUESTION,
            "Control mode?",
            "Position", "Cartesian Impedance", "Cartesian Sine Impedance", "Joint Impedance"
        );
        ...
        switch (usrin) {
          ...
          case 3:
            ...
            // Setup control mode
            controlMode = new JointImpedanceControlMode(
                jointStiffness,
                jointStiffness,
                jointStiffness,
                jointStiffness,
            ...
            for (int j = 0; j < 7; j++) {
              getLogger().info("    Joint axis " +String.valueOf(j)+ " stiffness: " + String.valueOf(stiffness[j]));
            }
            for (int j = 0; j < 7; j++) {
              getLogger().info("    Joint axis " +String.valueOf(j)+ " damping: " + String.valueOf(damping[j]));
            }
        ...
        getLogger().info("Connecting to FRI (timeout is " + String.valueOf(await_seconds) + " seconds) ...");
        try {
